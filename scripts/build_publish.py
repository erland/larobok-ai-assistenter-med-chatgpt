...
def postprocess_epub(epub: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="ai-assistenter-chatgpt-epub-") as tmp_name:
        tmp = Path(tmp_name)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ai-assistenter-chatgpt-pdf-") as tmp_name:
        tmp = Path(tmp_name)
